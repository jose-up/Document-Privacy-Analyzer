...
def segment_document(text: str, *, max_clause_chars: int = 1200) -> List[Clause]:
    """
    Deterministic segmentation:
        1) split on headings and numbered sections
        2) fallback: paragraph blocks
    Returns clauses with byte offsets into original text.
    """
    if not text:
        return []
        ...
        if len(chunk) > max_clause_chars:
            ...
            for j, (s, e) in enumerate(subs):
                clauses.append(Clause(
                    id=_stable_id(f"clause{i}.{j}", s, e),
                    span=Span(s, e),
                    text=text[s:e].strip()))
        else:
            clauses.append(Clause(
                id=_stable_id(f"clause{i}", start, end),
                span=Span(start, end),
                text=chunk))
...
def _split_large(text: str, start: int, end: int, max_clause_chars: int) -> List[Tuple[int, int]]:
    ...
    while cur < len(chunk):
        ...
        limit = min(len(chunk), cur + max_clause_chars)
